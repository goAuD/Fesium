...
- If you call `detach_width_request` on a label, grid that label with a sticky containing `e` and `w` in a weighted column. The two go together: detached and unstretched means Tk clips the text to one character.
