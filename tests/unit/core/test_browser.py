...
def test_open_local_url_rejects_out_of_range_port(monkeypatch):
    called = []
    monkeypatch.setattr(
        "fesium.core.browser.webbrowser.open",
        lambda url: called.append(url) or True,
    ...
    assert open_local_url("http://localhost:99999") is False
    assert called == []


def test_open_local_url_rejects_non_numeric_port(monkeypatch):
    called = []
    monkeypatch.setattr(
        "fesium.core.browser.webbrowser.open",
        lambda url: called.append(url) or True,
    ...
    assert open_local_url("http://localhost:abc") is False
    assert called == []
