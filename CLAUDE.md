...
- **Detaching a label's width and its sticky are one decision.** `detach_width_request` makes a label ask for a single character, which is only safe when the cell stretches it back: grid it with a sticky containing both `e` and `w`, in a weighted column. Detach it in a cell that does not stretch and Tk clips the text to about 8px - that is how every tile title and meta disappeared at once. See `ui/widgets/label_sizing.py`.
- **A label must never demand width.** A wrapped `CTkLabel` asks Tk for a width equal to its `wraplength`, so whatever width it is given once, it demands from then on. That ratchet defeats `BentoGrid`'s uniform columns - one long paragraph stretched its tile from 405px to 635px - and the tug of war between the demand and the grid's clamp is what makes a panel visibly shimmer. `BodyText` and `Tile`'s header labels set the inner label's `width=1`; width is the cell's decision, `wraplength` only decides where lines break.
