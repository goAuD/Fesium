...
- MySQL and MariaDB support in the `Database` view, reached over PyMySQL through a new engine seam (`core/database_engines.py`) rather than a bundled admin panel - [ADR 0002](docs/decisions/0002-mysql-through-our-own-view.md). The read-only gate, risk classification, transaction handling and result shape are unchanged from SQLite; only connecting, schema inspection, placeholders, identifier handling and the verb sets moved behind the engine boundary. Read-only now holds twice on MySQL: keyword gating plus `SET SESSION TRANSACTION READ ONLY` pinned at connect. The confirmation prompt is engine-aware too: in write mode MySQL's own state-changing verbs - `GRANT`, `REVOKE`, `CALL`, `RENAME`, `LOAD DATA` - now require it, where the gate previously knew only SQLite's vocabulary and let all five through unprompted while `DROP` and `TRUNCATE` were caught. `CREATE` deliberately does not prompt, because a warning that a query may remove data is one the user learns to click through when it is not true. A dead host fails fast - a 0.75s probe first, then a bounded 5s connect timeout. Connection failures are reported in plain words naming the host and port tried. The password is asked per session, held in one private controller attribute, never written to disk, never carried by any state object or its repr; `.env` reading still touches only `DB_CONNECTION`, `DB_HOST`, `DB_PORT` and `DB_DATABASE`. Deliberately not included: dump import/export, form-based row editing, user and privilege management.
