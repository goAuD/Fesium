...
- [x] **A macOS CI job stalled at random.** Five times, always `macos-latest`, moving between Python versions, always inside `Run tests`. `faulthandler_timeout = 60` turned the fifth stall into evidence within a minute: the hang was `root.update()` in `_settle` - Tk's event-loop pump, which on the macOS runner sometimes never returns. The suite now settles with `update_idletasks()` (same geometry pass, no window-server wait) and dispatches the one event the elision test needs by hand. The same call in the `Copy Setup Report` handler went with it - reachable from a display-gated test, and the next place the stall would have surfaced. If it ever stalls again, the traceback names the line within a minute.
